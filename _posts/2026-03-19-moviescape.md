...
## Making a Navigable UMAP of the top 90k movies
...
**Obtaining per-movie context vectors.** With the enriched per-movie JSONs in hand, proceeded to concatenate the fields of each json to create a single per-movie string (as shown below) and then convert these strings into vectors with [Voyage AI](https://www.voyageai.com/) (`voyage-3-large`, 512 dimensions). The deep fields dominate the context vectors because they have the most words, but the simpler fields (title, cast, genre) are also present and thus still searchable. Thus which movies end up as neighors is mostly driven by the deep fields
...
Running UMAP on 80,937 × 512 vectors takes less than 10 minutes on a laptop. After we got our 2D coordinates, we ran a two-level heirarchical k-means clustering on the points, yielding about 12 large-scale clusters and 80 sub-regions. Then we draw about 20 random sables from each of these regions and used Haiku to name each cluster based on that random samble (eg "French Art House", "Slasher Horror", "90s Action"). These labels appear at different zoom levels and are meant to help orient the reader.
...
The map is fairly useful for discovering new movies as well. Consider this example, where we start by zooming in on Terminator and then look at its (less popular) neighbors.
...
It is through spatial layouts like Moviescape that I hope AI will actually bring more signal to the internet rather than less.
