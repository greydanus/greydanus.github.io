...
layout: post
comments: true
title:  "How Simulating (Part II)"
excerpt: "How simulating the universe could give rise to quantum mechanics. A second and better essay on the topic."
...
First, it breaks local realism, which is actually a pair of constraints that say "things cannot be two things at once" and "things can only interact with their immediate neighbors". It breaks both of these: first, by definition, it says that quantum states exist in a superposition of states and second, it says that wavefunction collapse happens instantaneously, even when the system is spread across large distances.

Second, it permits large-scale entangled systems, such as a mole of entangled atoms, which require much more information than exists in the entire universe to instantiate and more compute than has ever happened in the universe to evolve. These are deep problems with quantum mechanics, and I have always felt that -- while its empirical results are reliably excellent -- something about the core ansatz must be wrong.
